...
interface MerchantActionModalsProps {
  ...
  onUpdatePurchasedPackage: (merchantId: string, packageData: PurchasedPackage) => Promise<void>;
}
...
type ModalAction = 
  ...
  | { type: 'RESET'; payload: { limits: MerchantLimits; statuses: MerchantStatuses } }
...
const MODAL_CONFIGS = {
  ...
  toggleStatuses: {
    title: "Toggle Badges",
    description: (name: string, visibility?: boolean, status?: string) => `Toggle Badges for ${name}.`,
  ...
  managePurchasedPackage: {
    ...
    description: (name: string, visibility?: boolean, status?: string) => `Update purchased package details for ${name}.`,
...
// State reducer
const modalStateReducer = (state: ModalState, action: ModalAction): ModalState => {
  switch (action.type) {
    ...
    case 'SET_PURCHASED_PACKAGE':
      return { ...state, purchasedPackage: { ...state.purchasedPackage, ...action.payload } };
...
const SuspensionReasonInput = memo(({ value, onChange }: { value: string; onChange: (value: string) => void }) => (
...
const LimitInputField = memo(({ label, value, onChange, placeholder }: { label: string; value: number; onChange: (val: number) => void; placeholder: string }) => (
...
const AdjustLimitsForm = memo(({ limits, secretCode, onLimitsChange, onSecretCodeChange }: { limits: MerchantLimits; secretCode: string; onLimitsChange: (key: keyof MerchantLimits, value: any) => void; onSecretCodeChange: (value: string) => void }) => (
...
const StatusToggleItem = memo(({ label, isEnabled, onChange }: { label: string; isEnabled: boolean; onChange: (checked: boolean) => void }) => (
  <div className="flex items-center justify-between p-4 border rounded-lg bg-gray-50 dark:bg-gray-800">
    <div className="flex-1">
      <h4 className="font-medium text-sm">{label}</h4>
      <Badge variant={isEnabled ? "default" : "destructive"} className={isEnabled ? "bg-green-500 text-white" : ""}>
...
const ToggleStatusesForm = memo(({ statuses, onStatusChange }: { statuses: MerchantStatuses; onStatusChange: (key: keyof MerchantStatuses, value: boolean) => void }) => (
  <div className="grid grid-cols-1 gap-4 max-h-[400px] overflow-y-auto">
...
const PackageInputField = memo(({ label, value, onChange, placeholder, type = "text" }: { label: string; value: string; onChange: (val: string) => void; placeholder: string; type?: string }) => (
  <div className="p-4 border rounded-lg bg-gray-50 dark:bg-gray-800">
    <label className="text-sm font-medium">{label}</label>
    <Input
      type={type}
...
const PackageSelectField = memo(({ label, value, onChange }: { label: string; value: string; onChange: (val: string) => void }) => (
  <div className="p-4 border rounded-lg bg-gray-50 dark:bg-gray-800">
    <label className="text-sm font-medium">{label}</label>
...
const ManagePurchasedPackageForm = memo(({ packageData, onPackageChange }: { packageData: PurchasedPackage; onPackageChange: (key: keyof PurchasedPackage, value: string) => void }) => (
  <div className="space-y-6 max-h-[400px] overflow-y-auto">
    <div className="p-4 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
      <h3 className="text-lg font-semibold text-blue-900 dark:text-blue-100 mb-4">Package Details</h3>
...
function MerchantActionModals({
  ...
  useEffect(() => {
    if (modal.type !== "deactivate" && !(modal.type === "confirmStatusChange" && modal.newStatus === "suspended")) {
      dispatch({ type: 'SET_SUSPENSION_REASON', payload: "" });
    }
    if (modal.type === "adjustLimits" && modal.merchant) {
      dispatch({ type: 'INITIALIZE_LIMITS', payload: initializeLimits(modal.merchant) });
    }
    if (modal.type === "toggleStatuses" && modal.merchant) {
      dispatch({ type: 'INITIALIZE_STATUSES', payload: initializeStatuses(modal.merchant) });
    }
    if (modal.type === "managePurchasedPackage" && modal.merchant) {
      dispatch({ type: 'INITIALIZE_PURCHASED_PACKAGE', payload: initializePurchasedPackage(modal.merchant) });
    }
    ...
    try {
      switch (modal.type) {
        ...
        case "activate":
          ...
          break;
        case "deactivate":
          await onUpdateMerchantStatus(modal.merchant._id, "suspended", state.suspensionReason);
          break;
        case "toggleVisibility":
          await onUpdateMerchantVisibility(modal.merchant._id, !modal.merchant.visibility);
          break;
      }
      onClose();
    } catch (error) {
  ...
  }, [modal.type, modal.merchant, state.suspensionReason, onUpdateMerchantStatus, onUpdateMerchantVisibility, onClose]);
    ...
    try {
      switch (modal.type) {
        case "adjustLimits":
          ...
          await onUpdateMerchantLimits(modal.merchant._id, state.limits, state.secretCode);
          break;
        case "toggleStatuses":
          await onUpdateMerchantStatuses(modal.merchant._id, state.statuses);
          break;
        case "confirmVisibilityChange":
          await onUpdateMerchantVisibility(modal.merchant._id, modal.newVisibility!);
          break;
        case "confirmStatusChange":
          await onUpdateMerchantStatus(modal.merchant._id, modal.newStatus!, modal.newStatus === "suspended" ? state.suspensionReason : undefined);
          break;
        case "managePurchasedPackage":
          await onUpdatePurchasedPackage(modal.merchant._id, state.purchasedPackage);
          break;
      }
      onClose();
    } catch (error) {
  ...
  }, [modal.type, modal.merchant, state.limits, state.secretCode, state.statuses, state.purchasedPackage, state.suspensionReason, modal.newVisibility, modal.newStatus, onUpdateMerchantLimits, onUpdateMerchantStatuses, onUpdateMerchantVisibility, onUpdateMerchantStatus, onUpdatePurchasedPackage, onClose]);
  ...
  const handleConfirmClick = useCallback(() => {
    if (modal.type === "adjustLimits" || modal.type === "toggleStatuses" || modal.type === "managePurchasedPackage") {
      dispatch({ type: 'SET_SHOW_CONFIRMATION', payload: true });
    } else if (modal.type === "confirmVisibilityChange" || modal.type === "confirmStatusChange") {
      handleMainConfirm();
    } else {
      handleSimpleConfirm();
    }
  ...
  const isConfirmDisabled = useMemo(() => {
    return (
      ((modal.type === "deactivate" || (modal.type === "confirmStatusChange" && modal.newStatus === "suspended")) && !state.suspensionReason.trim()) ||
  ...
  const handleLimitsChange = useCallback((key: keyof MerchantLimits, value: any) => {
    dispatch({ type: 'SET_LIMITS', payload: { [key]: value } });
  }, []);

  const handleStatusChange = useCallback((key: keyof MerchantStatuses, value: boolean) => {
    dispatch({ type: 'SET_STATUSES', payload: { [key]: value } });
  }, []);

  const handlePackageChange = useCallback((key: keyof PurchasedPackage, value: string) => {
    dispatch({ type: 'SET_PURCHASED_PACKAGE', payload: { [key]: value } });
  }, []);

  const content = useMemo(() => {
    if (!modal.merchant) return null;
    return getModalConfig(modal.type, modal.merchant.displayName, modal.newVisibility, modal.newStatus);
  }, [modal.type, modal.merchant?.displayName, modal.newVisibility, modal.newStatus]);
    ...
    <>
      <Dialog open={!!modal.type} onOpenChange={onClose}>
        <DialogContent>
          ...
          {(modal.type === "deactivate" || (modal.type === "confirmStatusChange" && modal.newStatus === "suspended")) && (
            <SuspensionReasonInput
              value={state.suspensionReason}
              onChange={(value) => dispatch({ type: 'SET_SUSPENSION_REASON', payload: value })}
            />
          )}
          {modal.type === "adjustLimits" && (
            <AdjustLimitsForm
              ...
              onSecretCodeChange={(value) => dispatch({ type: 'SET_SECRET_CODE', payload: value })}
            />
          )}
      ...
      <Dialog open={state.showConfirmation} onOpenChange={(open) => dispatch({ type: 'SET_SHOW_CONFIRMATION', payload: open })}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Confirm Update</DialogTitle>
            <DialogDescription>
              Are you sure you want to {modal.type === "adjustLimits" ? "update the limits" : modal.type === "toggleStatuses" ? "update the statuses" : modal.type === "managePurchasedPackage" ? "update the purchased package" : "change the visibility"} for {merchant.displayName}?
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="space-x-2">
            <Button variant="outline" onClick={() => dispatch({ type: 'SET_SHOW_CONFIRMATION', payload: false })}>
              Cancel
            </Button>
